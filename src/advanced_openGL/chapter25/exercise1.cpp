...
int main()
{
    ...
    unsigned int grassTexture = loadTexture("src/textures/red_window.png");
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        for(int i = 0; i < windows.size(); i++){
            float dist = glm::length(camera.cameraPos - windows[i]);
            sorted[dist] = i;
        }
        ...
        for(auto it = sorted.rbegin(); it != sorted.rend(); ++it){
            model = glm::mat4(1.0f);
            model = glm::translate(model, windows[it->second]);
            shader.setMat4("model", model);
            glDrawArrays(GL_TRIANGLES, 0, 6);
        }
