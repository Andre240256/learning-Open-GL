...
int main()
{
    ...
    //load texture
    unsigned char * emissionMapData = stbi_load("src/textures/emission_map.png", &width, &height,
                                            &nrChannels, 0);
    if(emissionMapData){
        GLenum format;
        if(nrChannels == 3)
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, 
                    0, format, GL_UNSIGNED_BYTE, emissionMapData);
        glGenerateMipmap(GL_TEXTURE_2D);
    }
    else{
        std::cout << "FAILED TO LOAD TEXTURE" << std::endl;
    ...
    stbi_image_free(emissionMapData);
